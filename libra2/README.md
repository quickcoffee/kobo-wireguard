...
## Modules
WireGuard requires the TUN/TAP device driver to be loaded to function.
...
## iptables
WireGuard requires the `iptables` binary and shared libraries to be present on the device to function.
These are not included in the stock Kobo Libra 2 kernel so this repo provides it as
pre-built binaries/libraries in the `binaries/iptables` directory.
...
## WireGuard binaries
The `wg` and `wg-quick` binaries need to be compiled for ARM and placed in the `binaries/wireguard` directory.
These can be built from [wireguard-tools](https://git.zx2c4.com/wireguard-tools/) source code using an ARM cross-compiler.